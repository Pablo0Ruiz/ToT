...
def get_post(id, check_author=True):
    post = get_db().execute(
        "SELECT p.id, p.title, p.body, p.body_html,  p.created, p.author_id, u.username,"
...
def index():
    db = get_db()
    posts = db.execute(
        "SELECT p.id, p.title, p.body, p.body_html, p.created, p.author_id, u.username, pi.filename AS image_filename "
        "FROM post p "
        "JOIN user u ON p.author_id = u.id "
        "LEFT JOIN post_image pi ON p.id = pi.post_id "
        "GROUP BY p.id "
